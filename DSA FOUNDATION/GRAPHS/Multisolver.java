...
public class Main {
    ...
    public static void multisolver(ArrayList<Edge>[] graph, int src, int dest, boolean[] visited, int criteria, int k,
        ...
        if (src == dest) {
            ...
            // for just smaller and just larger path(Ceil and floor)
            if (wsf < criteria) {
                if (wsf > fpathwt) {
                    fpathwt = wsf;
                    fpath = psf;
            ...
            if (pq.size() < k) {
                ...
                if (pq.peek().wsf < wsf) {
                    pq.remove(); // remove week element
                    pq.add(new Pair(wsf, psf)); // add stronger element
                }
            }
